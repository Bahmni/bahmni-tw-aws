...
sed -i '1i \'$aws_vault_key'\' /var/go/.keys/bahmni-tw-aws.pem
...
ansible-playbook -i inventory/ docker_instance.yml --vault-password-file ~/.keys/bahmni-tw-aws.pem --extra-vars="container_name=$container_name" --extra-vars="https_port=$https_port"  --extra-vars="erp_port=$erp_port" --extra-vars="aws_access_key=$aws_access_key" --extra-vars="aws_secret_key=$aws_secret_key" -vvv
cd ..
